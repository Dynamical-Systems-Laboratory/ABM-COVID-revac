										...
										const std::map<std::string, double>& infection_parameters, const Testing& testing)
{
	...
	if (agent.get_latency_end_time() <= time){
		...
		if (agent.recovering_exposed()){
			...
			// If contact traced - assign treatment and skip the testing
			if (agent.tested_covid_positive() == false && agent.contact_traced() == false){
				...
				// At this point agent is already removed from all places except 
				// for household so no need to repeat
					...
					std::vector<Transit>& public_transit)
{
	...
	if (agent.recovering() == true){
		if (agent.get_recovery_time() <= time){
			removed.at(0) = 1;
			if (agent.tested_false_negative() == false && !agent.contact_traced()){
			...
			const std::map<std::string, double>& infection_parameters)
{
	if (infection.agent_hospitalized(agent.get_age(), agent.severe_correction(time)) == true){
		...
		// Remove agent from all places, but then add to their
		// hospital for treatment (otherwise need to track the ID for more than one)
		remove_agent_from_all_places(agent, households, schools, hospitals, 
						carpools, public_transit);
