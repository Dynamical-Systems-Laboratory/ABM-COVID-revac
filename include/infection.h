...
public:
	...
	/// @param eff - vaccine effectiveness 
	bool infected(const double lambda, const double eff);
	...
	/// @param cor - correction for vaccinated
	bool recovering_exposed(const int age, const double cor);
	...
	/// @param va - correction for vaccinated w.r.t. being asymptomatic 
	/// @param vs - correction for vaccinated w.r.t. being hospitalized 
	/// @param vd - correction for vaccinated w.r.t. mortality 
	/// @param is_hsp - true if an agent is a hospital employee or a former non-COVID-19 patient
	bool will_die_non_icu(const int age, const double va, const double vs,  
							const double vd, const bool is_hsp = false);

	/// \brief Determine if the agent will be hospitalized
 	/// @param age - agents age
	/// @param vs - correction for vaccinated w.r.t. being hospitalized
	bool agent_hospitalized(const int age, const double vs);
	...
	/// Return a random number between d_min and d_max according to uniform distribution
	double get_uniform(const double d_min, const double d_max) { return rng.get_random(d_min, d_max); }
