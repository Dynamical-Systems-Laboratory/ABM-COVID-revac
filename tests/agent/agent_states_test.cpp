...
bool test_states_on_off()
{
	...
	if (!set_and_get(&Agent::set_symptomatic, &Agent::symptomatic, agent))
 	...
 	if (!set_and_get(&Agent::set_tested_covid_negative, &Agent::tested_covid_negative, agent))
		return false;
 	if (!set_and_get(&Agent::set_former_suspected, &Agent::former_suspected, agent))
		return false;
	...
	if (!set_and_get(&Agent::set_removed, &Agent::removed, agent))
		return false;
	if (!set_and_get(&Agent::set_removed_dead, &Agent::removed_dead, agent))
		return false;
	if (!set_and_get(&Agent::set_removed_recovered, &Agent::removed_recovered, agent))
		return false;
	if (!set_and_get(&Agent::set_vaccinated, &Agent::vaccinated, agent))
		return false;
	if (!set_and_get(&Agent::set_needs_next_vaccination, &Agent::needs_next_vaccination, agent))
		return false;
	if (!set_and_get(&Agent::set_removed_can_vaccinate, &Agent::removed_can_vaccinate, agent))
		return false;
	if (!set_and_get(&Agent::set_suspected_can_vaccinate, &Agent::suspected_can_vaccinate, agent))
		return false;
	if (!set_and_get(&Agent::set_more_active, &Agent::more_active, agent))
		return false;
	if (!set_and_get(&Agent::set_contact_traced, &Agent::contact_traced, agent))
		return false;
	return true;
}
