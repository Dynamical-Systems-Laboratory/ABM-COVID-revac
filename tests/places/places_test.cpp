...
{
	int pID = 1016; 
	double severity_cor = 1.5, beta = 0.149, psi = 0.25;
	std::string type = "carpool";

	Transit transit(pID, beta, severity_cor, psi, type);

	if (!general_place_test(transit, pID, 0.0, 0.0, severity_cor, beta, -1.0, -1.0, psi, {}, {}, type)){
		return false;
	}
...
bool contribution_test_transit()
{
	...
	double severity_cor = 2.25, beta = 1.49, psi = 0.5;
	double inf_var = 1.23;
	std::string type = "public";
	
	int n_exp = 3, n_sym = 2, n_tot = 10;
	double exp_lambda = 0.9622;
