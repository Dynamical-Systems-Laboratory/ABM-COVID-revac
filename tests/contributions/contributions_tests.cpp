...
bool contributions_main_test()
{
	...
	for (auto& agent : nc_agents){
		if (agent.infected() == false){
			if (static_cast<double>(std::rand())/static_cast<double>(RAND_MAX)<0.3) {
				abm.setup_traced_isolation({agent.get_ID()});	
			}
			continue;
		}
		...
		if (static_cast<double>(std::rand())/static_cast<double>(RAND_MAX)<0.55 
